...
> 🌐 Chinese Version: [README_FIN_cn.md](README_FIN_cn.md)
...
| $T_{year}$| Annual seconds | Fixed at 7257600 (calculated as 365 days × 24 hours × 3600 seconds) |
...
1. **Stock Positions**:  
   Market Value = Real-time price of the underlying stock × Position quantity  
   (Price source: `MarketData` generated by the GBM model)

2. **Option Positions**:  
   Market Value = Theoretical option price (calculated by Black-Scholes) × Position quantity × Contract multiplier  
...
# Stock expected return rate μ (used in GBM model)
portfolio.marketdata.mu.AAPL=0.08
portfolio.marketdata.mu.TELSA=0.12

# Stock volatility σ (shared by GBM and Black-Scholes models)
